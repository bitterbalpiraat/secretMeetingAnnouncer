...
def embedFactory(eventKey, timesIncluded=True, footerIncluded=True):
    ...
    if events[eventKey]['description'] is not None:
        embedVar = discord.Embed(
            title=events[eventKey]['subject'], description=events[eventKey]['description'], color=0xf570ff)
    else:
        embedVar = discord.Embed(
            title=events[eventKey]['subject'], description="*No description available*", color=0xf570ff)

    if footerIncluded:
        embedVar.set_footer(
            text="Would you like to participate in our ecosystem? Come join one of the weekly committee meetings!")
    ...
    if timesIncluded:  # calculate startTimes
        ...
        # create the embed
        embedVar.add_field(name="Start time in GMT/UTC",
                           value=startUTC.strftime("%H:%M"), inline=False)
        embedVar.add_field(name="New York", value=startUTC.astimezone(
            gettz('America/New_York')).strftime("%H:%M"), inline=True)
        embedVar.add_field(name="London",   value=startUTC.astimezone(
            gettz('Europe/London')).strftime("%H:%M"), inline=True)
        embedVar.add_field(name="Seoul",  value=startUTC.astimezone(
            gettz('Asia/Seoul')).strftime("%H:%M"), inline=True)
...
async def scheduleReminders(eventKey, secondsTillEvent):
    ...
    try:
        # query the channels dict with the classification
        committeeChannel = channels[events[eventKey]['classification']]
    except:
...
async def getCal():
    ...
    if 'importedEvents' in locals():  # check if importedEvents is loaded correctly in memory
        ...
        if len(importedEvents) > 0:
            ...
            for event in importedEvents:
                ...
                eventKey = event['startUTC-5']+'_' + \
                    event['subject']  # generate unique dict key
    ...
    events = {key: event for key,
              event in events.items() if event in importedEvents}
